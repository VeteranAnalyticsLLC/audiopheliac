...
export function ensureDbReady(): Promise<void> {
  // Production Vercel functions do not ship pglite.data. Skip the embedded
  // fallback there so a missing DATABASE_URL cannot crash the kit.
  if (dbSource !== "pglite" || import.meta.env.PROD) return Promise.resolve();
...
// Eager PGLite only in dev. The Vercel function does not ship pglite.data;
// throwing here would blank the deployed kit. Auth still opens PGLite on demand.
if (typeof window === "undefined" && dbSource === "pglite" && !import.meta.env.PROD) {
